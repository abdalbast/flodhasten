...
function IntroAnimation({ onComplete, isDarkMode }) {
  ...
  const [showTapToSkip, setShowTapToSkip] = useState(false);
  ...
  useEffect(() => {
    ...
    const testVideo = async () => {
      ...
      // iOS autoplay workaround: if video doesn't start in 2s, show skip/tap
      const timer = setTimeout(() => {
        if (videoRef.current && videoRef.current.paused) {
          setShowSkip(true);
          setShowTapToSkip(true);
          setIsLoading(false);
        }
      }, 2000);
      // Fallback: after 5s, skip intro if still not playing
        ...
        if (videoRef.current && videoRef.current.paused) {
          try {
            ...
            if (playPromise) {
              ...
              setShowTapToSkip(false);
            }
          } catch {
            startOutro();
          }
        } else if (showTapToSkip) {
          // If video is not paused but tap-to-skip is shown, skip
          startOutro();
        }
      }}
    >
      ...
      {showTapToSkip && !isOutro && (
        <div style={{
          ...
          Tap anywhere to start or skip the intro.
        </div>
      )}
